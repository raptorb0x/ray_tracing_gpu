...
bool scene_intersect(const Vec3f &orig, const Vec3f &dir, const std::vector<Sphere> &spheres, Vec3f &hit, Vec3f &N, Material &material) {
    ...
    if (fabs(dir.y)>1e-3)  {
        ...
        Vec3f pt = orig + dir*d;
        if (d>0 && fabs(pt.x)<20 && pt.z<0 && pt.z>-40 && d<spheres_dist) {
            checkerboard_dist = d;
            hit = pt;
            N = Vec3f(0,1,0);
...
Vec3f cast_ray(const Vec3f &orig, const Vec3f &dir, const std::vector<Sphere> &spheres, const std::vector<Light> &lights, size_t depth=0) {
    ...
    if (depth>r_depth || !scene_intersect(orig, dir, spheres, point, N, material)) {
...
void render(const std::vector<Sphere> &spheres, const std::vector<Light> &lights, uint8_t* pixmap) {
...
int main() {
    ...
    lights.push_back(Light(Vec3f( 30, 20,  30), 2));
    ...
    while (window.isOpen())
    {
        ...
        spheres[3].center = Vec3f(  8 * sin(i / 10.0) ,  -1 , -16 -8 * cos(i / 10.0));
        lights[0].position = Vec3f(-20-5*cos(i / 10.0), 20,  20);
        lights[1].position = Vec3f(-20, 20+5*cos(i / 10.0) ,  20);
        lights[2].intensity = 1 * fabs(tan(i/30.0));
        render(spheres, lights, pixmap);
